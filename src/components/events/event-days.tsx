      ...
      <ol className={cn("relative space-y-4", className)}>
        ...
        {days.map((d) => (
          <li key={d.day} className="relative flex gap-5">
            <span className={cn("relative z-10 flex size-10 shrink-0 items-center justify-center rounded-full font-mono text-sm font-semibold text-white shadow-md", d.showcase ? "bg-brand-coral" : "bg-gradient-to-br from-brand-blue to-brand-coral")}>{d.showcase ? <Trophy className="size-4" aria-hidden /> : d.day}</span>
    ...
    <ol className={cn("grid grid-cols-1 gap-2 sm:grid-cols-5", className)} aria-label="Five-day programme">
      {days.map((d, i) => (
        <li key={d.day} className={cn("relative flex items-center gap-3 rounded-2xl border px-3.5 py-3 backdrop-blur sm:flex-col sm:items-start sm:gap-2", d.showcase ? (light ? "border-brand-coral/50 bg-accent/60" : "border-brand-coral/60 bg-brand-coral/25") : light ? "border-border/80 bg-background" : "border-white/15 bg-white/10")}>
          ...
          <span className={cn("inline-flex items-center gap-1.5 font-mono text-xs font-semibold", light ? "text-brand-red-deep" : "text-brand-coral-light")}>{d.showcase && <Trophy className="size-3.5" aria-hidden />}{d.showcase ? "Showcase" : `Day ${d.day}`}</span>
