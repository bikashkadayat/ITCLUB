...
const cards = [
  { icon: Rocket, title: "Innovation Projects", text: "Building real-world solutions", pos: "left-[-6%] top-[8%]", delay: "0s" },
  { icon: BrainCircuit, title: "AI & Data Science", text: "Explore machine learning & AI", pos: "right-[-8%] top-[18%]", delay: "1.2s" },
  { icon: Code2, title: "Software Development", text: "Build production applications", pos: "left-[-10%] top-[52%]", delay: "2.1s" },
  { icon: ShieldCheck, title: "Cyber Security", text: "Secure the digital future", pos: "right-[-6%] top-[62%]", delay: "0.6s" },
  { icon: Trophy, title: "Leadership & Innovation", text: "Grow as future leaders", pos: "left-[14%] bottom-[-7%]", delay: "1.7s" },
];
    ...
    <div className="relative mx-auto w-full max-w-[560px] lg:max-w-none" aria-hidden>
      ...
      <div className="relative aspect-[5/4] overflow-hidden rounded-[2rem] border border-white/10 bg-brand-navy shadow-[0_40px_120px_-40px_var(--glow-blue)] ring-1 ring-brand-blue/30">
        ...
        <div className="absolute bottom-5 left-5 right-5 rounded-2xl border border-white/10 bg-brand-ink/60 p-4 text-white backdrop-blur">
          <p className="text-[10px] font-semibold uppercase tracking-[0.25em] text-brand-coral-light">Students building technology</p>
          <p className="mt-1 font-display text-lg leading-tight">AI · Software · Security · Data</p>
      ...
      {/* floating cards */}
      {cards.map((c) => (
        <div key={c.title} className={`hero-float absolute ${c.pos} hidden w-[210px] items-start gap-3 rounded-2xl border border-border/80 bg-card/90 p-3 shadow-[0_20px_50px_-20px_var(--glow-blue)] backdrop-blur sm:flex`} style={{ animationDelay: c.delay }}>
          ...
          <span className="min-w-0"><span className="block text-[13px] font-medium leading-tight">{c.title}</span><span className="block text-[11px] text-muted-foreground">{c.text}</span></span>
        </div>
      ))}
    </div>
  );
