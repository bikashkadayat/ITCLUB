...
const cards = [
  { thumb: { name: "nepalNetwork" as const }, title: "Innovation Projects", pos: "left-[-6%] top-[8%]", delay: "0s" },
  { thumb: { name: "aiHologram" as const }, title: "AI & Data Science", pos: "right-[-8%] top-[18%]", delay: "1.2s" },
  { thumb: { name: "connectedDevices" as const }, title: "Software Development", pos: "left-[-10%] top-[52%]", delay: "2.1s" },
  { thumb: { name: "serverRoom" as const }, title: "Cyber Security", pos: "right-[-6%] top-[62%]", delay: "0.6s" },
  { thumb: { src: "/images/team/bikash-kadayat.jpg" }, title: "Student Leadership", pos: "left-[14%] bottom-[-7%]", delay: "1.7s" },
];
  ...
  return (
    <div className="relative mx-auto w-full max-w-[560px] lg:max-w-none" aria-hidden>
      ...
      <div className="relative aspect-[5/4] overflow-hidden rounded-[2rem] border border-white/10 bg-brand-navy shadow-[0_40px_120px_-40px_var(--glow-blue)] ring-1 ring-brand-blue/30">
        <Image src="/images/gallery/computer-lab-session.jpg" alt="" fill priority sizes="(min-width: 1024px) 50vw, 100vw" className="object-cover object-[60%_40%]" />
        <div className="absolute inset-0 bg-gradient-to-tr from-brand-navy/55 via-brand-blue/20 to-transparent mix-blend-multiply" />
        <div className="absolute inset-0 bg-gradient-to-t from-brand-ink/85 via-brand-ink/5 to-transparent" />
        {/* circuit / network overlay */}
        <svg className="absolute inset-0 h-full w-full opacity-30" viewBox="0 0 500 400" fill="none" preserveAspectRatio="none">
      ...
      {cards.map((c) => (
        <div key={c.title} className={`hero-float absolute ${c.pos} hidden items-center gap-3 rounded-2xl border border-border/80 bg-card/90 py-2.5 pl-2.5 pr-4 shadow-[0_20px_50px_-20px_var(--glow-blue)] backdrop-blur sm:flex`} style={{ animationDelay: c.delay }}>
          <Thumb {...c.thumb} className="size-9" />
